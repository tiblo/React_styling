...
객체에서의 Spread Operator 사용은 3가지 용도로 사용한다.
- 객체의 복사(property를 복사한 새 객체 생성)
- 새로운 property를 추가한 새 객체 생성(기존 객체에 새 property를 추가하는 경우도 해당됨)
- property를 업데이트 한 객체 생성(기존 객체의 property 업데이트 하는 경우도 해당됨)
